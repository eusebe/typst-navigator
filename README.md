...
### 0.1.8
- **Bug fix**: `get-active-headings` could no longer tell headings apart from a `here()` call once every location collapsed to page 1 / y 0pt — true both under plain HTML export and, more surprisingly, *inside* `html.frame(..)` even though `target()` still reports `"paged"` there. It degenerated to "the last heading always wins". Since `query()` still returns headings in document order in both cases, `get-active-headings` now first looks for `loc` by identity among the headings it already queried and, if found, reads its index instead of its position — this is what makes `target-location: h.location()` (already the recommended pattern for Touying/hook contexts) reliable under HTML export too. The common `here()`-without-a-matching-heading case is unchanged. No regression: all existing examples/docs/tests recompile to byte-for-byte identical PDF text; also verified against `typst-sorbonne-presentation` and `typst-presentate`'s structured themes (280+ files recompiled fresh against old and new navigator, zero diffs).
- **New examples**: [Typstage](examples/typstage_transitions.typ) transitions and [miniframes](examples/typstage_miniframes.typ) integrations. Typstage parses headings into its own data before layout, so a `show heading: ...` never fires; both examples drop a hidden heading per section so navigator has something real to query, then pass its location explicitly via `target-location:` (the fix above is what makes that reliable in the HTML build). The miniframes example goes further: `render-miniframes`/`get-structure` associate headings with slides by page number, which collapses to a single group under `html.frame(..)` — sidestepped entirely by reading typstage's own `info()` for "which section is this slide in" and feeding the matching hidden heading's location to `progressive-outline` instead.
- **New helpers**: `mark-slide()` emits the `metadata((t: "ContentSlide"))` marker navigator's default `slide-selector` looks for, instead of every integration writing out that literal by hand. `is-transition-here()` factors out the page-based "is this a transition/divider page" check that `diatypst_integration.typ` used to hand-roll, reading the mapped levels from `navigator-config` instead of hardcoding them.
